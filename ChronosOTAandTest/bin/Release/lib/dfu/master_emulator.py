...
class MasterEmulator(object):
    ...
    def connection_update_handler(self, sender, e):
        """ Callback for the DLL if connection update request is received. """
        conn_interval = e.ConnectionIntervalMinMs
        conn_timeout = e.ConnectionSupervisionTimeoutMs
        identifier = e.Identifier
        slave_latency = e.SlaveLatency
        self.master.SendConnectionUpdateResponse(identifier, Nordicsemi.ConnectionUpdateResponse.Accepted)

        connection_params = Nordicsemi.BtConnectionParamters()
        connection_params.ConnectionIntervalMs = conn_interval
        connection_params.SlaveLatency = slave_latency
        connection_params.ConnectionSupervisionTimeoutMs = conn_timeout
        self.master.UpdateConnectionParameters(connection_params)
    ...
    def send_data(self, pipe, msg, count):
    ...
    def scan_and_connect(self, emulator_filter=""):
        ...
        try:
            ...
            self.master.ConnectionUpdateRequest += self.connection_update_handler

            emulator_devices = self.master.EnumerateUsb(Nordicsemi.UsbDeviceType.AnyDevice)
